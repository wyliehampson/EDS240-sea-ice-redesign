...
final_plot <- ggplot(sea_ice_by_month, aes(date, avg_vol)) +
  geom_line(aes(group = month, color = label)) +
  geom_line(data = sea_ice_mean, size = 1) +
  geom_label(data = sea_ice_mean %>% filter(date == max(date)),
             aes(label = label),
             size = 3,
             nudge_x = 700) +
  ...
  scale_color_manual(labels = c("January", "February", "March", "April", "May",
                                "June", "July", "August", "September", "October",
                                "November", "December"),
                     values = c("#b63434", "#b63434", "#601ea1", "#248df0", "#0cb3a7", "#138261",
